...
int qgsGridUtil::genGrids(QgsGeometry bndGeometry, int srcEpsgId, int distance, QgsVectorLayer* lyr, GridType gridType)
{
	...
	if (!lyr) //如果没有指定数据存储图层，则建立内存数据存储图层
		lyr = qgsGeoUtils::createMemLayer(Qgis::WkbType::Polygon, "EPSG:" + srcEpsgId, QString::fromLocal8Bit("抽样网格"), fields);

	if (!lyr)return -1;

	QgsVectorLayer* gridPointLayer = qgsGeoUtils::createMemLayer(Qgis::WkbType::Point, QString("EPSG:%1").arg(srcEpsgId),
