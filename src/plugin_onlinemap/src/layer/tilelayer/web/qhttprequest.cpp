...
namespace geotile {
	...
	qHttpRequest::qHttpRequest(char type, LONGLONG id, QObject* parent) :
		QObject(parent)
	{
		m_type = type;
		m_tileid = id;
	...
	//发起请求
	void qHttpRequest::sendRequest(const QString& strUrl)
	{
		m_strUrl = strUrl;
	...
	void qHttpRequest::postRequest(const QString& strUrl, const QString& posUrl)
	{
		m_strUrl = strUrl;
	...
	//请求结束
	void qHttpRequest::slot_requestFinished()
	{
		m_pTimer->stop();//关闭定时器
	...
	//请求超时
	void qHttpRequest::slot_requestTimeout()
	{
	...
	/////////////////////////////////////////////////////////
	// ----------数据下载模块
	QgsWmsTiledImageDownloadHandler::QgsWmsTiledImageDownloadHandler(const qTileRequests& requests, qTileLayerRender* pRender)
		: mRender(pRender)
		, mEventLoop(new QEventLoop)
	{

		Q_FOREACH(const qTileRequest & r, requests)
		{
	...
	/////////////////////////////////////////////////////////
	// ----------数据下载模块
	qEdomDownloadHandler::qEdomDownloadHandler(QgsPolygonXY* geo,
				const qTileRequests& requests, qTileDb* TileStore)
		: mEventLoop(new QEventLoop)
	{
	...
	qEdomDownloadHandler::~qEdomDownloadHandler()
	{
		delete mEventLoop;
	}

	void qEdomDownloadHandler::downloadBlocking()
	{
	...
	void qEdomDownloadHandler::tileReplyFinished()
	{
	...
	void qEdomDownloadHandler::CutImage(LONGLONG tileReqNo, QByteArray& buf)
	{
		GEO_RECT tileRect = qTileUtils::GetGeoRectById(tileReqNo);
		QgsPolygonXY imgPolygon;
		QgsPolylineXY res;
		res.resize(5);
		...
		imgPolygon.push_back(res);
		QgsGeometry geo = QgsGeometry::fromPolygonXY(imgPolygon);
		...
		*/
		QgsGeometry qgsGeometry = QgsGeometry::fromPolygonXY(*pDataBoundPolygon);
		if (qgsGeometry.isEmpty())return;
		//判断多边形是否相交
		if (qgsGeometry.contains(geo)) //如果外包矩形包含了
		{
		...
		int polycount = pDataBoundPolygon->size(); //找出线段数
		for (int pc = 0; pc < polycount; pc++)
		{
			QgsPolylineXY line = pDataBoundPolygon->at(pc);
			int lx, ly, point_count = line.size();
			QPolygon poly;
			for (int n = 0; n < point_count; n++)
			{
				double dx = line.at(n).x();
				double dy = line.at(n).y();
	...
	void qEdomDownloadHandler::cancelled()
	{
		QgsDebugMsg("Caught cancelled() signal");
		Q_FOREACH(QNetworkReply * reply, mReplies)
		{
	...
	void qEdomDownloadHandler::repeatTileRequest(QNetworkRequest const& oldRequest)
	{
		/*	QgsWmsStatistics::Stat& stat = QgsWmsStatistics::statForUri(mProviderUri);
