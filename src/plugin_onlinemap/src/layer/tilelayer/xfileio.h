...
public:
	...
	bool readString(wstring &s)
	{
		//读入一个dword长度
		EVDWORD cnt;
		if(read(&cnt, sizeof(EVDWORD)) != sizeof(EVDWORD))
			return false;

		//读入数据
		s= L"";
		for(EVDWORD i = 0; i < cnt; i++)
		{
			unsigned short c;
	...
	bool writeString(wstring &s)
	{
		//读入一个dword长度
		EVDWORD cnt = s.size();
		if(write(&cnt, sizeof(EVDWORD)) != sizeof(EVDWORD))
			return false;
...
/************************************************************************/
#ifndef evULONG
#define  evULONG unsigned long
#endif
#ifndef evUINT
#define  evUINT unsigned int
#endif
...
#define  SAVEVECTOR(a, t) {\
	EVDWORD sz = a.size();\
	RETURNFALSE2(io.write(&sz, sizeof(EVDWORD)) == sizeof(EVDWORD));\
	if(sz > 0){\
...
#define  LOADVECTOR(a, t) {\
	EVDWORD sz;\
	RETURNFALSE2(io.read(&sz, sizeof(EVDWORD)) == sizeof(EVDWORD));\
	if(sz > 0 && sz < (EVDWORD)io.MAXBUFMEMSIZE){\
	a.resize(sz);\
...
//计算结构数据大小
#define CALCULSIZE_BEGIN    EVDWORD datasize;bool CaculSize(){datasize=0;
