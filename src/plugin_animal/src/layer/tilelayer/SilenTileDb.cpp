...
SilenTileDb::SilenTileDb(QString dbPath,const EV_DB_Header& tileInfo,bool CreateNew)
...
SilenTileDb::~SilenTileDb()
{
	close();
}

//关闭瓦片存储数据库
bool SilenTileDb::close()
{
...
//开启数据库写入事务
bool SilenTileDb::startTransaction(){
	if(!m_database.isOpen() && !m_database.open())
		return false;
	return m_database.transaction();
}
//关闭并提交事务
bool SilenTileDb::endTransaction(){
	if(!m_database.isOpen() && !m_database.open())
		return false;
	return m_database.commit();
}

bool SilenTileDb::save(LONGLONG tileId,QByteArray td){
...
bool SilenTileDb::save(QList<SaveBlockStru> *blocks)
{//批量保存
	//数据库不是打开的且不能打开则返回false表明不能写入数据
	if (!m_database.isOpen() && !m_database.open())
		return false;
...
//根据层块编号删除一个瓦片
bool SilenTileDb::del(LONGLONG tileId){
	//数据库不是打开的且不能打开则返回false表明不能写入数据
	if(!m_database.isOpen()&&!m_database.open())
		return false;
...
bool SilenTileDb::ExitTile(LONGLONG tileId)
...
QByteArray SilenTileDb::read(LONGLONG tileId)
{
	//数据库不是打开的且不能打开则返回false表明不能写入数据
	if (!m_database.isOpen() && !m_database.open())
		return NULL;
...
bool SilenTileDb::SaveSysTile(const EV_DB_Header &tileInfo){
	//数据库不是打开的且不能打开则返回false表明不能写入数据
	if(!m_database.isOpen()&&!m_database.open())
		return false;
...
bool SilenTileDb::SaveDataInfo(QString myear, QString mgdcode, QString mname, QString malias,QString mzt)
{
	//数据库不是打开的且不能打开则返回false表明不能写入数据
	if(!m_database.isOpen()&&!m_database.open())
		return false;
...
void SilenTileDb::initdb(bool CreateNew)
{
	if(CreateNew)
	{
...
void SilenTileDb::ExeSQL(QString sql)
{
	if (!m_database.isOpen() && !m_database.open())
		return;
