	...
	:mQgsInterface(qgsInterface)
{
	...
	addLayerMenuAction(mLayerRepaireAction, QgsMapLayerType::VectorLayer);
	...
	addLayerMenuAction(lyrRepair, QgsMapLayerType::VectorLayer);
}
...
void qgsLayerContextMenu::addLayerMenuAction(QAction* action, QgsMapLayerType lyrType)
{
	//!创建图层修复工具按钮
	if (mQgsInterface)
...
void qgsLayerContextMenu::on_LayerAdded(QgsMapLayer* l)
{
	Q_FOREACH(QAction * act, mLayerActions.keys())
	{
		QgsMapLayerType actLyrType = mLayerActions.value(act);
		QgsMapLayerType ltype = l->type();
